...
- extractedClientName: 請求先（宛先・お客様）の会社名・名前。請求書に「請求先」「宛先」等の欄があればその値を抽出。なければ空文字でよい。
...
以下のJSON形式のみで回答してください（JSON以外は出力しないでください）：
{
  ...
  "extractedClientName": "抽出した請求先（宛先）の会社名・名前。記載がなければ空文字",
...
export async function extractAndCheckInvoicePdf(
  ...
  try {
    ...
    const vendorName = typeof raw.extractedVendorName === "string"
      ...
      : "";
    const clientName = typeof raw.extractedClientName === "string"
      ? String(raw.extractedClientName).trim()
      : "";
    ...
    return { vendorName, clientName: clientName || undefined, targetMonth, aiResult };
  } catch (error) {
